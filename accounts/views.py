...
def lastlogin(request):
    if request.POST.get("tcno") or tc:
    ...
    elif request.POST.get('tc'):
        if Coach.objects.filter(person__tc=request.POST.get('tc')):
            coach = Coach.objects.filter(person__tc=request.POST.get('tc'))[0]
            user = User.objects.get(pk=coach.user.pk)
            user_form = UserForm(request.POST or None, instance=coach.user)
            person_form = PersonForm(request.POST, request.FILES, instance=coach.person)
            communication_form = CommunicationForm(request.POST or None, instance=coach.communication)
            iban_form = IbanCoachForm(request.POST or None, instance=coach)
            metarial_form = MaterialForm(request.POST)
            mail = request.POST.get('email')
        ...
        elif Judge.objects.filter(person__tc=request.POST.get('tc')):
            judge = Judge.objects.filter(person__tc=request.POST.get('tc'))[0]
            user_form = UserForm(request.POST or None, instance=judge.user)
            person_form = PersonForm(request.POST or None, request.FILES or None, instance=judge.person)
            communication_form = CommunicationForm(request.POST or None, instance=judge.communication)
            metarial_form = MaterialForm(request.POST or None, instance=judge.person.material)
            iban_form = IbanFormJudge(request.POST or None, instance=judge)
            name = request.POST.get('first_name')
            surname = request.POST.get('last_name')
            year = request.POST.get('birthDate')
